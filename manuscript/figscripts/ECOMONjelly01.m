function [Z_GB, Z_GOM, Z_NY, Z_MAB] = ECOMONjelly01()
...
figure('position',[1     1   610   548])
lbls={'(A)','(B)','(C)'};
for s=1:length(species)
    subplot(length(species),1,s)
    plot(years,TS(:,:,s),'color',[.5 .5 .5])
    hold on
    plot(years,nanmean(TS(:,:,s),2),'linewidth',3,'color',[0 0 .5])
    set(gca,'fontsize',16,'linewidth',3)
    title(specnames(s),'fontsize',12)
    if s==2
        ylabel('Proportion presence')
    end
    axis([1975 2025 0 1])
    set(gca,'ytick',[0 1])
    text(1976.2,0.9,lbls(s))
end